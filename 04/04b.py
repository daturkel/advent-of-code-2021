...
class Board:
    ...
    def check_for_win_last_turn(self) -> bool:
        if self.won:
            return False

        # we assume # of rows = # of columns
        for i in range(len(self.numbers[0])):
            self.won = self.won or self._check_row(i)
            self.won = self.won or self._check_column(i)
            if self.won:
                break
...
if __name__ == "__main__":
    ...
    for (i, number), board in product(enumerate(numbers), boards):
        ...
        if i >= 5 and board.check_for_win_last_turn():
            num_wins += 1
        if num_wins == len(boards):
            break
